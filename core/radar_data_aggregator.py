...
# sys.path.append("/wk171/handsomedong/DLRA_paper/rainfall_prediction-master/")
...
### Chia Tung for only 2018
    def run(self, workers):
        for month in os.listdir(self._sdir):
            print(month)
            month_dir = os.path.join(self._sdir, month)
            for day in tqdm(list(os.listdir(month_dir))):
                day_dir = os.path.join(month_dir, day)
                arguments = []
                # for fname in os.listdir(day_dir):
...
class CompressedAggregatedRadarData:
    ...
    def load_from_raw(raw_data):
        data = CompressedAggregatedRadarData.INVALID_VALUE * np.ones((NX, NY), dtype=np.float32)
        if len(raw_data) == 0:
            return data
