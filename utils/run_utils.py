...
from models.adverserial_model_attention_hetero import BalAdvAttenHeteroModel
from models.adverserial_SCAN import Adv_SCAN
...
from models.forecaster import Forecaster, Forecaster_PONI
...
def get_model(
    ...
    if model_type == ModelType.BaselineCNN:
        ...
        if model_type in [
                ...
                ModelType.SSIMGRUModel, ModelType.BalancedGRUAdvPONI, ModelType.CapnHetero,ModelType.Adv_SCAN,
        ]:
            # 建立encoder和decoder
            ...
            if model_type in [ModelType.BalancedGRUAdvPONI, ModelType.CapnHetero]:
        ...
        if data_kwargs['hetero_data'] is True: ###for HETERO
            checkpoint_dict['hetr'] = 1

        if data_kwargs['SCAN_data'] is True:####for SCAN
            checkpoint_dict['scan'] = 1
        ...
        elif model_type == ModelType.CapnHetero:
            assert data_kwargs['data_type'] == DataType.Radar + DataType.Rain
            model = BalAdvAttenHeteroModel(
                model_kwargs['adv_w'],
                model_kwargs['dis_d'],
                encoder,
                forecaster,
